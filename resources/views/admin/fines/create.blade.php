...
<div class="admin-shell">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ isset($fine) ? route('admin.fines.update', $fine->id) : route('admin.fines.store') }}"
                  method="POST" novalidate>
                @csrf
                @if(isset($fine)) @method('PUT') @endif
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="regulator" name="regulator" class="form-select @error('regulator') is-invalid @enderror">
                            ...
                            @foreach([
                                ...
                                <option value="{{ $r }}" {{ old('regulator', isset($fine) ? $fine->regulator : '') == $r ? 'selected' : '' }}>{{ $r }}</option>
                            @endforeach
                        </select>
                        @error('regulator')
                            <span class="invalid-feedback">{{ $message }}</span>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="sector" name="sector" class="form-select @error('sector') is-invalid @enderror">
                            ...
                            @foreach([
                                ...
                                <option value="{{ $s }}" {{ old('sector', isset($fine) ? $fine->sector : '') == $s ? 'selected' : '' }}>{{ $s }}</option>
                            @endforeach
                        </select>
                        @error('sector')
                            <span class="invalid-feedback">{{ $message }}</span>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="region" name="region" class="form-select @error('region') is-invalid @enderror">
                            ...
                            @foreach(['EU / EEA','USA','Australia','Canada','Global'] as $reg)
                                <option value="{{ $reg }}" {{ old('region', isset($fine) ? $fine->region : '') == $reg ? 'selected' : '' }}>{{ $reg }}</option>
                            @endforeach
                        </select>
                        @error('region')
                            <span class="invalid-feedback">{{ $message }}</span>
                ...
                <div class="mb-3">
                    ...
                    <select id="law" name="law" class="form-select @error('law') is-invalid @enderror">
                        ...
                        @foreach(['GDPR','UK GDPR','DPA 2018','CCPA','Other'] as $l)
                            <option value="{{ $l }}" {{ old('law', isset($fine) ? $fine->law : '') == $l ? 'selected' : '' }}>{{ $l }}</option>
                        @endforeach
                    </select>
                    @error('law')
                        <span class="invalid-feedback">{{ $message }}</span>
                ...
                <div class="mb-3">
                    ...
                    <select id="violation_type" name="violation_type" class="form-select @error('violation_type') is-invalid @enderror">
                        ...
                        @foreach([
                            ...
                            <option value="{{ $v }}" {{ old('violation_type', isset($fine) ? $fine->violation_type : '') == $v ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
                ...
                <div class="d-flex gap-2 mt-4">
                    <button type="submit" class="btn btn-primary">{{ isset($fine) ? 'Update' : 'Create' }} Fine</button>
